...
class SetupWizard:
    ...
    def _select_language(self) -> str:
        ...
        selected_language = prompt_for_choice(
    ...
    def _category_based_selection(self, language: str) -> str:
        ...
        while True:
            ...
            action = prompt_for_choice(
                ...
                ["Browse a category", "Finish selection", "Start over"]
            )
            ...
            if action == "Finish selection":
                ...
                category_libs = self.tech_stack_manager.search_libraries(
                    language=language,
                ...
                lib_choice = prompt_for_choice(
                    f"\\nSelect a {category} library (or skip)",
                    lib_names + ["Skip this category"]
                )
    ...
    def _generate_recommendations(self, language: str, game_type: str, experience: str, scope: str) -> List[Tuple[str, str]]:
        ...
        if language == "Lua":
            recommendations.append((
                "Love2D",
                "Perfect for 2D games in Lua, beginner-friendly with great documentation"
            ))
        
        elif language == "Python":
            recommendations.append((
                "Pygame",
                "Excellent for learning game development, great for 2D games and prototypes"
            ))
            
            if "3D" in game_type:
                recommendations.append((
                    "Pygame+NumPy",
                    "Pygame with NumPy for better math operations in 3D"
                ))
            ...
            if experience == "Beginner":
                if "2D" in game_type:
                    recommendations.append((
                        "SDL2+OpenGL",
                        "Good starting point for 2D/3D graphics with moderate complexity"
                    ))
                else:
                    recommendations.append((
                        "SDL2+OpenGL+GLM",
                        "Complete 3D starter stack with windowing, rendering, and math"
                    ))
            else:  # Intermediate/Advanced
                if "3D" in game_type:
                    recommendations.append((
                        "SDL2+OpenGL+GLM+Assimp+stb_image",
                        "Full 3D game development stack with asset loading"
                    ))
                ...
                if "2D" in game_type:
                    recommendations.append((
                        "SDL2+OpenGL+GLM+stb_image",
                        "Modern 2D development with OpenGL rendering"
                    ))
